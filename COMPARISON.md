...
## 18. MatrixOne vs Neon vs Supabase（Backend-as-a-Service 场景）

**先分层**：**Neon** = serverless Postgres（数据库基础设施，主打 **DB 分支** + PITR + 自动伸缩/缩到零）；
**Supabase** = Postgres 之上的**完整 BaaS**（DB + 自动 REST/GraphQL API + Auth + Realtime + 对象 Storage
+ Edge Functions + Row-Level Security + Studio + 客户端 SDK）；**MatrixOne** = HTAP 数据库（MySQL 兼容），
即**数据库层**本身。

| 能力 | **Neon** | **Supabase** | **MatrixOne** |
|---|---|---|---|
| 内核 | serverless Postgres | Postgres | HTAP（MySQL 兼容） |
| **DB 分支**（CoW） | ✅ 整库 CoW，分支=独立 endpoint | ✅(基于 Neon/分支) | ✅ `CLONE`/`DATA BRANCH`（实测 1000+5000 行 185ms）|
| 分支**行级 diff/merge** | ❌（自己写 SQL） | ❌ | ✅ `DATA BRANCH DIFF/MERGE/PICK`（实测 INSERTED=100）|
| PITR / 时间旅行 | ✅(WAL) | ✅ | ✅ snapshot/PITR（+git4data 版本化）|
| serverless 自动伸缩/缩到零 | ✅(招牌) | 部分(托管计算) | 云版有弹性，无 per-branch endpoint/scale-to-zero |
| 自动 REST / GraphQL API | △(Data API) | ✅(PostREST + GraphQL) | ❌（需自建 API 层）|
| Auth / JWT / OAuth | ❌ | ✅(GoTrue) | ❌（只有 RBAC 角色/权限）|
| Realtime 推送（订阅变更） | ❌ | ✅(Realtime) | ❌（有 publish/subscribe 数据共享、CDC 更新版才有）|
| 对象 Storage（带策略） | ❌ | ✅(S3 + 策略) | △ `STAGE`/`datalink` 引用 OSS（无策略/无上传 API）|
| Edge Functions | ❌ | ✅(Deno) | ❌（有 SQL UDF；Python UDF 需起服务）|
| Row-Level Security | ❌ | ✅(RLS 策略) | ❌（实测 `CREATE POLICY` 语法错）|
| 向量/AI | ✅(pgvector) | ✅(pgvector) | ✅(`vecf32` + l2/cosine) |
| 客户端 SDK | JS 等 | JS/Flutter 等全套 | 官方 Python DB SDK（非 BaaS SDK）|
| Studio/Admin UI | ✅ | ✅ | ❌（用 SQL 客户端）|

**实测**（`exp_neon_branching.py`）：把 prod 库零拷贝 `CLONE` 出一个 dev 分支（185ms），在分支上
跑 schema 迁移(`ALTER ADD COLUMN`)+数据变更，**prod 完全不受影响**（隔离），`DATA BRANCH DIFF`
行级看出分支新增 100 行，最后 `DROP DATABASE` 丢弃分支（46ms）——这正是 **Neon 的 dev/preview/CI 分支
工作流**。两点诚实注脚：① 改了 schema 的表不能行级 diff（DIFF 需同 schema）；② 零拷贝 `CLONE` 无分支
血缘，DIFF 能认增/改但**认不出删**，要全量行级 diff/merge（含删）须用 `DATA BRANCH CREATE`（带 LCA）。

**究竟差多少**：
- **对标 Neon（可分支的 serverless DB）：差距小**。MatrixOne 已有"即时 CoW 分支 + PITR + 真 SQL 库 + 向量"，
  而且**行级 diff/merge 是 Neon 没有的加成**。主要差在 **serverless 形态**：Neon 每个分支是一个**独立、自动
  伸缩、可缩到零**的 Postgres endpoint（天然适配 branch-per-PR CI），MatrixOne 分支活在**同一实例内**（库/表级）。
- **对标 Supabase（完整 BaaS）：差距大**——缺的是**整个应用层**：自动 REST/GraphQL API、Auth、Realtime 推送、
  带策略的对象 Storage、Edge Functions、**RLS**、Studio、客户端 SDK（实测 RLS/CDC 在 v3.0.11 都没有）。
  MatrixOne 提供的是**数据库层**（外加 pub/sub 数据共享、SQL UDF、`STAGE`(OSS 文件引用)、向量、git4data）；
  要做成 Supabase 那样，得在它上面**自建/拼装**这整套应用层服务（就像本仓库 `durable_exec/` 在库上拼出 mini-DBOS 那样）。
