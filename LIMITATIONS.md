...
|3.4|🟨 **无 TTL 子句**（`CREATE TABLE … TTL …` 语法错）|§15.1 实测|无 `TTL` 列子句；但 **v3.0.11 已支持内置 `CREATE TASK` 定时任务**（实测可用），可定时 `DELETE` 旧数据模拟过期。注：`CREATE TASK` 的 `BEGIN…END` 体内含分号，标准 CLI 需切 DELIMITER 或用驱动整条发送，否则会被 `;` 截断误报语法错|
